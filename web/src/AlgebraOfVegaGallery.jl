...
data(summary) * mapping(:median, y=:parameter) *
    ...
    ("bruno_pi_resolve_independent", "Bruno QT — PI picker + raw resolve indep (regression)",
     "Mirrors web-pkpd/qt.jl _render_posterior: horizontal pointinterval through with_plot_caption(auto_remap=...), " *
     "row=:basename, raw config(resolve=Dict(\"scale\"=>Dict(\"x\"=>\"independent\",\"y\"=>\"independent\"))). " *
     "When the picker collapses :basename to a single value, the spec ends up non-faceted; the user's " *
     "facet-intended resolve was previously inherited at top level and made VL emit per-layer x-axes " *
     "(stacked at the chart top). Now scrubbed by _scrub_independent_resolve_if_unfaceted!.",
